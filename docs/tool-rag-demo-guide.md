...
4. ツール選択をまだ変更していない受講者が設定画面を開き、Web検索がオンになったことを確認する。
5. すでにツール選択を変更したブラウザセッションでは、その受講者の選択が維持されることを確認する。
6. どのセッションでもWeb検索とRAGが常に表示され、受講者が自由にオン・オフできることを確認する。
7. デモ後は管理者のツール設定を両方オフに戻す。
